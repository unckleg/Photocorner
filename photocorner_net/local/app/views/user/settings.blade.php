...
@section('content')
   ...
   <div class="form-group">
      <label for="username">{{ t('Username') }}
         <small>( Korisničko ime ne može biti izmenjeno )</small>
      </label>
      {{ Form::text('username',$user->username,['class'=>'form-control','disabled'=>'disabled'])}}
   </div>
   <div class="form-group">
      <label for="fullname">{{ t('Fullname') }}</label>
      {{ Form::text('fullname',$user->fullname,['class'=>'form-control','required'=>'required']) }}
   </div>
   <div class="form-group">
      <label for="gender">{{ t('Gender') }}<small>*</small></label>
      {{ Form::select('gender', ['male' => 'Male', 'female' => 'Female'], Auth::user()->gender, ['id'=>'gender','class'=>'form-control','required'=>'required']) }}
   </div>
   <div class="form-group">
      <label for="country">{{ t('Country') }}
         <small>( Država u kojoj živite )</small>
      </label>
   ...
   <div class="form-group">
      <label for="country">{{ t('Date Of Birth') }}
         <small>( godina - mesec - datum )</small>
      </label>
      {{ Form::text('dob',$user->dob,['class'=>'form-control','id'=>'dp1']) }}
   </div>
   <div class="form-group">
      <label for="about_me">{{ t('About Me') }}
         <small>( Par reči o Vama )</small>
      </label>
      {{ Form::textarea('about_me',$user->about_me,['class'=>'form-control']) }}
   </div>
   <div class="form-group">
      <label for="blogurl">{{ t('Facebook Profile') }}
         <small>( Vaš Facebook profil ili Link )</small>
      </label>
      {{ Form::text('fb_link',$user->fb_link,['class'=>'form-control']) }}
   </div>
   <div class="form-group">
      <label for="blogurl">{{ t('Twitter Profile') }}
         <small>( Vaš Twitter profil ili Link )</small>
      </label>
      {{ Form::text('tw_link',$user->tw_link,['class'=>'form-control']) }}
   </div>
   <div class="form-group">
      <label for="blogurl">{{ t('Blog url') }}
         <small>( Vaš Blog ili Link bloga )</small>
      </label>
      {{ Form::text('blogurl',$user->blogurl,['class'=>'form-control']) }}
   </div>
   {{ Form::submit(t('Update'),['class'=>'btn btn-default btn-success']) }}
   {{ Form::close() }}
   <h3 class="content-heading">{{ t('Email Settings') }}</h3>
   {{ Form::open(['url' => 'settings/mailsettings']) }}
   <div class="form-group">
      <label for="emailcomment">{{ t('Email me when someone comment') }}</label>
      {{ Form::checkbox('emailcomment',1,$user->email_comment) }}
   </div>
   <div class="form-group">
      <label for="emailreply">{{ t('Email me when someone reply on my comment') }}</label>
      {{ Form::checkbox('emailreply',1,$user->email_reply) }}
   </div>
   <div class="form-group">
      <label for="emailfavorite">{{ t('Email me when someone favorte my image') }}</label>
      {{ Form::checkbox('emailfavorite',1,$user->email_favorite) }}
   </div>
   <div class="form-group">
      <label for="emailfollow">{{ t('Email me when someone follow me') }}</label>
      {{ Form::checkbox('emailfollow',1,$user->email_follow) }}
   </div>
   <div class="form-group">
   ...
   <div class="form-group">
      {{ Form::submit(t('Change Password'),['class'=>'btn btn-default btn-success']) }}
   </div>
   {{ Form::close() }}
@stop